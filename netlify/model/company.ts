...
async function getAllCompanies(): Promise<CompanyInformation[]> {
    try {
        let sql = `SELECT * FROM ${tableName} ORDER BY company_name ASC`;
        const [rows] = await conn.query<(CompanyInformation & RowDataPacket)[]>(sql);
        return rows;
    } catch (error) {
...
async function getCompanyById(id: number): Promise<CompanyInformation | null> {
    try {
        ...
        const [rows] = await conn.execute<(CompanyInformation & RowDataPacket)[]>(sql, [id]);
...
async function checkCompany(websiteUrl: string): Promise<boolean> {
    try {
        ...
        const [record] = await conn.execute<({ count: number } & RowDataPacket[])>(sql, [websiteUrl.toLowerCase()]);
